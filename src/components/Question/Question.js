...
const Question = ({ question }) => {
    ...
    useEffect(() => {
        document.addEventListener('click', function (event) {
            ...
            const newSelectedItem = event.target.innerText;
    ...
    const calculation = (question) => {
        if (selectedItem.replace(/\s/g, "") === question.correctAnswer.replace(/\s/g, "")) {
            toast.success("Correct Answer !");
        }
        else if (selectedItem.replace(/\s/g, "") !== question.correctAnswer.replace(/\s/g, "")) {
            toast.error("Wrong Answer !");
        }
        console.log(selectedItem.replace(/\s/g, ""), question.correctAnswer.replace(/\s/g, ""));
    };
    ...
    return (
        <div className='shadow-xl rounded mb-6 p-12 md:w-[80%] lg:w-[60%] m-auto text-left relative'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                {
                    options.map(option => <p key={option} className='bg-purple-100 hover:bg-purple-200 rounded p-6' onClick={() => calculation(question)}>{option}</p>)
                }
            </div>
